...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make Cude.new a better tool for developers worldwide. 💡
...
- Clone the repository:
  ```bash
  git clone https://github.com/cude-new/cude.new.git
  ```
...
```bash
docker run -p 5173:5173 --env-file .env.local cude:development
```
...
```bash
docker build . --target cude:production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local cude:production
```
