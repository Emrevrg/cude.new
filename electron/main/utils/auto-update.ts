// Cude.new - auto-update.ts (Cude product surface, 2026)
...
export async function setupAutoUpdater() {
  ...
  /*
   * A build with no update channel has no `electron-update.yml`, and the
   * updater's own logger reports the resulting ENOENT as an error before it
   * ever reaches our catch. Every launch of a healthy unsigned build therefore
   * opened with a stack trace about a file it was never meant to have.
   *
   * Only that one case is downgraded; anything else the updater says is
   * genuinely worth reading and still arrives as an error.
   */
  autoUpdater.logger = {
    ...logger,
    error: (...args: unknown[]) => {
      ...
      if (text.includes('ENOENT') && text.includes('electron-update.yml')) {
        logger.debug('No update channel is configured for this build.');
        return;
      }
  ...
  try {
    ...
    /*
     * A build with no update channel has no `app-update.yml`, and looking for
     * one throws ENOENT. That is the ordinary state of an unsigned local
     * build, and reporting it as an error meant every launch of a perfectly
     * healthy package opened with a failure and a stack trace.
     *
     * Anything else is a real problem and still says so.
     */
