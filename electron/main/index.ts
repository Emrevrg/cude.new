// Cude.new - index.ts (Cude product surface, 2026)
/// <reference types="vite/client" />
import { createRequestHandler } from '@remix-run/node';
import electron, { app, BrowserWindow, dialog, ipcMain, protocol, session } from 'electron';
...
(async () => {
  await app.whenReady();
  if (process.platform === 'win32') app.setAppUserModelId('com.cude.new');
  ...
  if (!isDev && !serverBuild) throw new Error('Desktop server build could not be loaded.');
  const productionHandler = serverBuild ? createRequestHandler(serverBuild, 'production') : undefined;
  ...
  protocol.handle('http', async (req) => {
    ...
    try {
      ...
      if (!productionHandler) throw new Error('Desktop server is unavailable.');
      console.log('Handling request with server build:', req.url);

      const result = await productionHandler(req, {
        /*
  ...
  .catch((error) => {
    ...
    dialog.showErrorBox('Cude.new could not start', error instanceof Error ? error.message : String(error));
    app.exit(1);
  });
