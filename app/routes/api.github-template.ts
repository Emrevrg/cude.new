// Cude.new - api.github-template.ts (Cude product surface, 2026)
import { json } from '@remix-run/cloudflare';
import JSZip from 'jszip';

/** An expired optional credential must not break public starter repositories. */
export async function fetchGitHub(url: string, githubToken?: string): Promise<Response> {
  const headers = { Accept: 'application/vnd.github.v3+json', 'User-Agent': 'Cude.new' };
  const response = await fetch(url, {
    headers: { ...headers, ...(githubToken ? { Authorization: `Bearer ${githubToken}` } : {}) },
...
async function fetchRepoContentsCloudflare(repo: string, githubToken?: string) {
  ...
  const repoResponse = await fetchGitHub(`${baseUrl}/repos/${repo}`, githubToken);
  ...
  const treeResponse = await fetchGitHub(
    `${baseUrl}/repos/${repo}/git/trees/${encodeURIComponent(defaultBranch)}?recursive=1`,
    githubToken,
  );
  ...
  for (let i = 0; i < files.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (file: any) => {
      try {
        const contentResponse = await fetchGitHub(
          `${baseUrl}/repos/${repo}/contents/${file.path.split('/').map(encodeURIComponent).join('/')}`,
          githubToken,
        );
        ...
        const bytes = Uint8Array.from(atob(contentData.content.replace(/\s/g, '')), (char) => char.charCodeAt(0));
...
async function fetchRepoContentsZip(repo: string, githubToken?: string) {
  ...
  const releaseResponse = await fetchGitHub(`${baseUrl}/repos/${repo}/releases/latest`, githubToken);
  ...
  const releaseData = releaseResponse.ok ? ((await releaseResponse.json()) as { tag_name?: string }) : null;

  // Templates without a published release are valid; use their default branch.
  const zipballUrl = `${baseUrl}/repos/${repo}/zipball${releaseData?.tag_name ? `/${encodeURIComponent(releaseData.tag_name)}` : ''}`;

  // Fetch the zipball
  const zipResponse = await fetchGitHub(zipballUrl, githubToken);
...
export async function loader({ request, context }: { request: Request; context: any }) {
  ...
  if (
    ...
    return json({ error: 'Use a GitHub repository in owner/name format.' }, { status: 400 });
  }
