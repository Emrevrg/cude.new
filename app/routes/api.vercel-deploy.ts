// Cude.new - api.vercel-deploy.ts (Cude product surface, 2026)
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    if (!detectedFramework && sourceFiles) {
      ...
      logger.debug('Framework detected from the source files', detectedFramework);
    }

    // If no projectId provided, create a new project
    if (!targetProjectId) {
      const projectName = `cude-${chatId}-${Date.now()}`;
