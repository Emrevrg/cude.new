// Cude.new - api.git-proxy.$.ts (Cude product surface, 2026)
...
async function handleProxyRequest(request: Request, path: string | undefined) {
  try {
     ...
     *
     * A git URL carries tokens in the query on some hosts, and this line went
     * to the server log on every request.
     */
    logger.debug('Proxying git request', `${request.method} https://${domain}/${remainingPath}`);
     ...
     *
     * `authorization` is on the allow list — it has to be, or a private
     * repository cannot be cloned — and this printed the whole map, so every
     * clone wrote the caller's git credential into the server log in plain
     * text. Which headers were forwarded is the diagnosable part; what was in
     * them is not ours to record.
     */
    logger.debug('Forwarding headers', [...headers.keys()].join(', '));
    ...
    logger.debug('Response headers', [...responseHeaders.keys()].join(', '));
