// Cude.new - VercelDeploy.client.tsx (Cude product surface, 2026)
...
export function useVercelDeploy() {
  ...
  const vercelConn = useServiceConnection('vercel');
  const currentChatId = useStore(chatId);

  const handleVercelDeploy = async () => {
    if (!vercelConn.isConnected || !vercelConn.connection) {
    ...
    try {
      ...
      const response = await fetch('/api/vercel-deploy', {
        ...
        body: JSON.stringify({
          ...
          token: vercelConn.connection.token,
  ...
  return {
    isDeploying,
    handleVercelDeploy,
    isConnected: vercelConn.isConnected,
  };
}
