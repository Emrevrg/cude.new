// Cude.new - GitLabDeploy.client.tsx (Cude product surface, 2026)
...
import { readStored } from '~/lib/cude/state/browserStore';
...
export function useGitLabDeploy() {
  ...
  const handleGitLabDeploy = async () => {
    const connection = readStored<Record<string, any>>('gitlab_connection');
    ...
    try {
      ...
      return {
        success: true,
        files: fileContents,
        projectName: artifact.title || 'cude-project',
      };
    } catch (err) {
  ...
  return {
    isDeploying,
    handleGitLabDeploy,
    isConnected: !!readStored<Record<string, any>>('gitlab_connection')?.user,
  };
}
