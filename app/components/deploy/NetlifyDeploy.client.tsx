// Cude.new - NetlifyDeploy.client.tsx (Cude product surface, 2026)
...
import { conversationId as chatId } from '~/lib/cude/state/useConversationHistory';
import { formatBuildFailureOutput } from './deployUtils';
import { createScopedLogger } from '~/utils/logger';

const logger = createScopedLogger('netlify-deploy');

export function useNetlifyDeploy() {
  const [isDeploying, setIsDeploying] = useState(false);
  const netlifyConn = useServiceConnection('netlify');
  const currentChatId = useStore(chatId);

  const handleNetlifyDeploy = async () => {
    if (!netlifyConn.isConnected || !netlifyConn.connection) {
    ...
    try {
      ...
      const deployArtifact = workbenchStore.artifactFor(deploymentId, deploymentId);
      ...
      logger.debug('Original build path', buildPath);
      ...
      for (const dir of commonOutputDirs) {
        try {
          ...
          logger.debug(`No ${dir} directory, trying the next candidate.`, error);
          continue;
        }
      }
      ...
      const response = await fetch('/api/netlify-deploy', {
        ...
        body: JSON.stringify({
          ...
          files: fileContents,
          token: netlifyConn.connection.token,
      ...
      while (attempts < maxAttempts) {
        try {
          const statusResponse = await fetch(
            ...
            {
              headers: {
                Authorization: `Bearer ${netlifyConn.connection.token}`,
              },
            },
          );
  ...
  return {
    isDeploying,
    handleNetlifyDeploy,
    isConnected: netlifyConn.isConnected,
  };
}
