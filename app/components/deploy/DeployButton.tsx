// Cude.new - DeployButton.tsx (Cude product surface, 2026)
...
import { DeploymentLink } from '~/components/cude/chat/DeploymentLink';
import { conversationId } from '~/lib/cude/state/useConversationHistory';
import { useVercelDeploy } from '~/components/deploy/VercelDeploy.client';
import { useNetlifyDeploy } from '~/components/deploy/NetlifyDeploy.client';
import { useGitHubDeploy } from '~/components/deploy/GitHubDeploy.client';
import { useGitLabDeploy } from '~/components/deploy/GitLabDeploy.client';
import { PublishDialog } from '~/components/cude/chat/PublishDialog';
...
export const DeployButton = ({
  ...
  const netlify = useServiceConnection('netlify');
  const vercel = useServiceConnection('vercel');
  const gitlabIsConnected = useServiceConnection('gitlab').isConnected;
  const currentConversationId = useStore(conversationId);
  ...
  return (
    <>
      <div className="flex border border-cude-borderColor rounded-md overflow-hidden text-sm">
        <DropdownMenu.Root>
          <DropdownMenu.Trigger
            disabled={isDeploying || !activePreview || isStreaming}
            className="rounded-md items-center justify-center [&:is(:disabled,.disabled)]:cursor-not-allowed [&:is(:disabled,.disabled)]:opacity-60 px-3 py-1.5 text-xs bg-cude-button-primary-background text-cude-button-primary-text hover:text-cude-item-contentAccent [&:not(:disabled,.disabled)]:hover:bg-cude-button-primary-backgroundHover outline-cude-borderColorActive flex gap-1.7"
          >
          ...
          >
            <DropdownMenu.Item
              className={classNames(
                ...
                {
                  'opacity-60 cursor-not-allowed': isDeploying || !activePreview || !netlify.isConnected,
                },
              )}
              disabled={isDeploying || !activePreview || !netlify.isConnected}
            ...
            >
              ...
              <span className="mx-auto">
                {!netlify.isConnected ? 'No Netlify Account Connected' : 'Deploy to Netlify'}
              </span>
              {netlify.isConnected && currentConversationId && (
                <DeploymentLink service="netlify" conversationId={currentConversationId} />
              )}
            </DropdownMenu.Item>
            ...
            <DropdownMenu.Item
              className={classNames(
                'cursor-pointer flex items-center w-full px-4 py-2 text-sm text-cude-textPrimary hover:bg-cude-item-backgroundActive gap-2 rounded-md group relative',
                {
                  'opacity-60 cursor-not-allowed': isDeploying || !activePreview || !vercel.isConnected,
                },
              )}
              disabled={isDeploying || !activePreview || !vercel.isConnected}
            ...
            >
              ...
              />
              <span className="mx-auto">
                {!vercel.isConnected ? 'No Vercel Account Connected' : 'Deploy to Vercel'}
              </span>
              {vercel.isConnected && currentConversationId && (
                <DeploymentLink service="vercel" conversationId={currentConversationId} />
              )}
            </DropdownMenu.Item>
            ...
            <DropdownMenu.Item
              disabled
              className="flex items-center w-full rounded-md px-4 py-2 text-sm text-cude-textTertiary gap-2 opacity-60 cursor-not-allowed"
            >
              <img
                className="w-5 h-5"
                height="24"
