// Cude.new - TabsWithSlider.tsx (Cude product surface, 2026)
...
export function TabsWithSlider({
  ...
  return (
    <div className={classNames('relative flex gap-2', className)}>
      ...
      {tabs.map((tab, index) => (
        <button
          ...
          className={classNames(
            'px-4 py-2 h-10 rounded-lg transition-all duration-200 flex items-center gap-2 min-w-[120px] justify-center relative overflow-hidden border',
            tab.id === activeTab
              ? classNames(
                  'bg-cude-textPrimary text-cude-background-depth-1 border-cude-textPrimary shadow-sm',
                  activeTabClassName,
                )
              : classNames(
                  'bg-cude-background-depth-2 text-cude-textPrimary border-cude-borderColor hover:bg-cude-background-depth-3',
                  tabClassName,
                ),
          )}
        >
      ...
      {/* Animated slider */}
      <motion.div
        className={classNames('absolute bottom-0 left-0 h-10 rounded-lg bg-cude-textPrimary -z-10', sliderClassName)}
        initial={false}
        animate={{
