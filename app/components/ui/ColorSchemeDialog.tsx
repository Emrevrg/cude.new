// Cude.new - ColorSchemeDialog.tsx (Cude product surface, 2026)
  ...
  const renderColorSection = () => (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-cude-textPrimary flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-cude-item-contentAccent"></div>
          Color Palette
        </h3>
        <button
          onClick={handleReset}
          className="text-sm bg-transparent hover:bg-cude-background-depth-2 text-cude-textSecondary hover:text-cude-textPrimary rounded-lg flex items-center gap-2 transition-all duration-200"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {paletteRoles.map((role) => (
          <div
            key={role.key}
            className="group flex items-center gap-4 p-4 rounded-xl bg-cude-background-depth-3 hover:bg-cude-background-depth-2 border border-transparent hover:border-cude-borderColor transition-all duration-200"
          >
            <div className="relative flex-shrink-0">
              <div
                className="w-12 h-12 rounded-xl shadow-md cursor-pointer transition-all duration-200 hover:scale-110 ring-2 ring-transparent hover:ring-cude-borderColorActive"
              ...
              <input
                ...
                tabIndex={-1}
              />
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-cude-background-depth-1 rounded-full flex items-center justify-center shadow-sm">
                <span className="i-ph:pencil-simple text-xs text-cude-textSecondary" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-cude-textPrimary transition-colors">{role.label}</div>
              <div className="text-sm text-cude-textSecondary line-clamp-2 leading-relaxed">{role.description}</div>
              <div className="text-xs text-cude-textTertiary font-mono mt-1 px-2 py-1 bg-cude-background-depth-1 rounded-md inline-block">
                {palette[role.key]}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
        {designFonts.map((f) => (
          <button
            ...
            className={`group p-4 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cude-borderColorActive ${
              font.includes(f.key)
                ? 'bg-cude-item-backgroundAccent border-cude-borderColorActive shadow-lg'
                : 'bg-cude-background-depth-3 border-cude-borderColor hover:border-cude-borderColorActive hover:bg-cude-background-depth-2'
            }`}
          >
            <div className="text-center space-y-2">
              <div
                className={`text-2xl font-medium transition-colors ${
                  font.includes(f.key) ? 'text-cude-item-contentAccent' : 'text-cude-textPrimary'
                }`}
              ...
              <div
                className={`text-sm font-medium transition-colors ${
                  font.includes(f.key) ? 'text-cude-item-contentAccent' : 'text-cude-textSecondary'
                }`}
              >
                {f.label}
              </div>
              {font.includes(f.key) && (
                <div className="w-6 h-6 mx-auto bg-cude-item-contentAccent rounded-full flex items-center justify-center">
          ...
          return (
            <div key={f.key} className="feature-card-container p-2">
              <button
                ...
                className={`group relative w-full p-6 text-sm font-medium transition-all duration-200 bg-cude-background-depth-3 text-cude-item-textSecondary ${
                  f.key === 'rounded'
                    ? isSelected
                      ? 'rounded-3xl'
                      : 'rounded-xl'
                  ...
                  f.key === 'border'
                    ? isSelected
                      ? 'border-3 border-cude-borderColorActive bg-cude-item-backgroundAccent text-cude-item-contentAccent'
                      : 'border-2 border-cude-borderColor hover:border-cude-borderColorActive text-cude-textSecondary'
                    : f.key === 'gradient'
                      ? ''
                      : isSelected
                        ? 'bg-cude-item-backgroundAccent text-cude-item-contentAccent shadow-lg'
                        : 'bg-cude-background-depth-3 hover:bg-cude-background-depth-2 text-cude-textSecondary hover:text-cude-textPrimary'
                ...
                style={{
                  ...(f.key === 'gradient' && {
                    background: isSelected
                      ...
                      : 'var(--cude-bg-depth-3)',
                    color: isSelected ? 'white' : 'var(--cude-textSecondary)',
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-cude-background-depth-1 bg-opacity-20">
                    {f.key === 'rounded' && (
                      <div
                    ...
                    {f.key === 'gradient' && (
                      <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-gray-400 via-pink-400 to-gray-400 opacity-90" />
                    )}
                    {f.key === 'shadow' && (
                    ...
                    {f.key === 'frosted-glass' && (
                      <div className="relative">
                        <div
                          className={`w-6 h-6 rounded-lg transition-all duration-200 backdrop-blur-sm bg-cude-background-depth-1/20 border border-white/30 ${
                            isSelected ? 'opacity-90' : 'opacity-70'
                          }`}
                        />
                        <div
    ...
    <div>
      ...
      <DialogRoot open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <Dialog>
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              <DialogTitle className="text-2xl font-bold text-cude-textPrimary">Design Palette & Features</DialogTitle>
              <DialogDescription className="text-cude-textSecondary leading-relaxed">
            ...
            {/* Navigation Tabs */}
            <div className="flex gap-1 p-1 bg-cude-background-depth-3 rounded-xl">
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-200 ${
                    activeSection === tab.key
                      ? 'bg-cude-background-depth-3 text-cude-textPrimary shadow-md'
                      : 'bg-cude-background-depth-2 text-cude-textSecondary hover:text-cude-textPrimary hover:bg-cude-background-depth-2'
                  }`}
                >
            ...
            {/* Action Buttons */}
            <div className="flex justify-between items-center">
              <div className="text-sm text-cude-textSecondary">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  variant="ghost"
                  onClick={handleSave}
                  className="bg-cude-button-primary-background hover:bg-cude-button-primary-backgroundHover text-cude-button-primary-text"
                >
                  Save Changes
                </Button>
              </div>
      ...
      <style>{`
        .custom-scrollbar {
          ...
          scrollbar-color: var(--cude-textTertiary) transparent;
        }
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background-color: var(--cude-textTertiary);
          border-radius: 3px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background-color: var(--cude-textSecondary);
        }
        .line-clamp-2 {
          display: -webkit-box;
          -webkit-line-clamp: 2;
