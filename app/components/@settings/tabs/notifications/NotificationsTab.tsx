// Cude.new - NotificationsTab.tsx (Cude product surface, 2026)
...
const NotificationsTab = () => {
  ...
  const filteredLogs = Object.values(logs)
    .filter((log) => {
      ...
      if (filter === 'provider') {
        return log.source === 'provider';
      }

      if (filter === 'network') {
        return log.source === 'workspace';
      }
  ...
  const getNotificationStyle = (level: string, type?: string) => {
    if (type === 'update') {
      return {
        ...
        color: 'text-cude-textTertiary',
        bg: 'hover:bg-cude-background-depth-3',
      };
    }
    ...
    switch (level) {
      case 'error':
        return {
          ...
          color: 'text-cude-item-contentDanger dark:text-red-400',
          bg: 'hover:bg-cude-item-contentDanger/10 dark:hover:bg-cude-item-contentDanger/20',
        };
      case 'warning':
        return {
          icon: 'i-ph:warning',
  ...
  const renderNotificationDetails = (details: NotificationDetails) => {
    if (details.type === 'update') {
      return (
        <div className="flex flex-col gap-2">
          <p className="text-sm text-cude-textSecondary">{details.message}</p>
    ...
    return details.message ? <p className="text-sm text-cude-textSecondary">{details.message}</p> : null;
  };

  const filterOptions: { id: FilterType; label: string; icon: string; color: string }[] = [
    { id: 'all', label: 'All Notifications', icon: 'i-ph:bell', color: '#6b7280' },
    { id: 'system', label: 'System', icon: 'i-ph:gear', color: '#6b7280' },
    { id: 'update', label: 'Updates', icon: 'i-ph:arrow-circle-up', color: '#6b7280' },
    ...
    { id: 'network', label: 'Network', icon: 'i-ph:wifi-high', color: '#0891b2' },
  ];
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            ...
            >
              ...
              <span className="i-ph:caret-down text-lg text-cude-textTertiary" />
            </button>
          </DropdownMenu.Trigger>

          <DropdownMenu.Portal>
            <DropdownMenu.Content
              className="min-w-[200px] bg-cude-background-depth-1 rounded-lg shadow-lg py-1 z-[250] animate-in fade-in-0 zoom-in-95 border border-cude-borderColor"
              sideOffset={5}
              align="start"
              side="bottom"
            >
              {filterOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.id}
                  className="group flex items-center px-4 py-2.5 text-sm text-cude-textSecondary hover:bg-cude-background-depth-3 cursor-pointer transition-colors"
        ...
        >
          <span className="i-ph:trash text-lg text-cude-textTertiary group-hover:text-gray-500 transition-colors" />
          Clear All
        </button>
      </div>
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-cude-textPrimary">No Notifications</h3>
              <p className="text-sm text-cude-textTertiary">You're all caught up!</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => {
            const style = getNotificationStyle(
              log.level,
              typeof log.detail?.type === 'string' ? log.detail.type : undefined,
            );
            return (
              <motion.div
                key={log.id}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <h3 className="text-sm font-medium text-cude-textPrimary">{log.message}</h3>
                      {log.detail && renderNotificationDetails(log.detail as NotificationDetails)}
                  ...
                  <time className="shrink-0 text-xs text-cude-textTertiary">
