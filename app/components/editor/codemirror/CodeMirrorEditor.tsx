// Cude.new - CodeMirrorEditor.tsx (Cude product surface, 2026)
...
/*
 * The workspace is the single authority on locks. The editor previously
 * consulted a second, separately persisted lock store, so a file could be
 * editable here while the workspace refused to write it.
 */
const isFileLocked = (filePath: string) => workbenchStore.isFileLocked(filePath);
...
function getReadOnlyTooltip(state: EditorState) {
  ...
  if (currentDoc?.filePath) {
    const { locked } = isFileLocked(currentDoc.filePath);
