// Cude.new - ToolInvocations.tsx (Cude product surface, 2026)
import type { ToolInvocationUIPart } from '@ai-sdk/ui-utils';
import { AnimatePresence, motion } from 'framer-motion';
import { memo, useMemo, useState, useEffect } from 'react';
import { getHighlighter } from '~/lib/cude/highlight';
...
/*
 * Highlighting arrives with the grammar, which is fetched on first use rather
 * than shipped with the page. Until then the JSON is shown as it is.
 */
function JsonBlock({ className, code, theme }: { className?: string; code: string; theme: Theme }) {
  ...
  useEffect(() => {
    ...
    getHighlighter(['json'])
      .then((highlighter) => {
        if (current) {
          setHtml(highlighter.codeToHtml(code, { lang: 'json', theme: theme === 'dark' ? 'dark-plus' : 'light-plus' }));
        }
      })
      .catch(() => undefined);
  ...
  if (html === null) {
    return (
      <pre className={classNames('text-xs rounded-md overflow-auto m-0 p-0 mcp-tool-invocation-code', className)}>
        {code}
      </pre>
    );
  }
...
function JsonCodeBlock({ className, code, theme }: JsonCodeBlockProps) {
  ...
  return <JsonBlock className={className} code={formattedCode} theme={theme} />;
}
...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-cude-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-cude-background-depth-2 hover:bg-cude-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Collapse details' : 'Expand details'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-cude-textSecondary hover:text-cude-textPrimary transition-colors"></div>
          </div>
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-cude-textPrimary font-medium leading-5 text-sm">
              MCP Tool Invocations{' '}
              {hasToolResults && (
                <span className="w-full w-full text-cude-textSecondary text-xs mt-0.5">
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-cude-artifacts-background hover:bg-cude-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-cude-textSecondary hover:text-cude-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-cude-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-cude-background-depth-2">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-cude-actions-background">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                {isErrorResult ? (
                  <div className="text-lg text-cude-icon-error">
                    <div className="i-ph:x"></div>
                  </div>
                ) : (
                  <div className="text-lg text-cude-icon-success">
                    <div className="i-ph:check"></div>
                  </div>
                )}
                <div className="text-cude-textSecondary text-xs">Server:</div>
                <div className="text-cude-textPrimary font-semibold">{annotation?.serverName}</div>
              </div>

              <div className="ml-6 mb-2">
                <div className="text-cude-textSecondary text-xs mb-1">
                  Tool: <span className="text-cude-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-cude-textSecondary text-xs mb-1">
                  Description:{' '}
                  <span className="text-cude-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-cude-textSecondary text-xs mb-1">Parameters:</div>
                <div className="bg-cude-background-depth-2 border border-cude-borderColor p-3 rounded-md">
                  <JsonCodeBlock className="mb-0" code={JSON.stringify(tool.toolInvocation.args)} theme={theme} />
                </div>
                <div className="text-cude-textSecondary text-xs mt-3 mb-1">Result:</div>
                <div className="bg-cude-background-depth-2 border border-cude-borderColor p-3 rounded-md">
      ...
      <ul className="list-none space-y-4">
        {toolInvocations.map((tool, index) => {
          ...
          return (
            ...
            >
              <div className="bg-cude-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  <div className="flex flex-col items-center ">
                    <span className="mr-auto font-light font-normal text-md text-cude-textPrimary rounded-md">
                      {toolName}
                    </span>
                    <span className="text-xs text-cude-textSecondary font-light break-words max-w-64">
                  ...
                  <div className="flex items-center justify-end gap-2 ml-auto">
                    <button
                      className={classNames(
                        ...
                        'bg-transparent',
                        'text-cude-textTertiary hover:text-cude-textPrimary',
                    ...
                    <button
                      className={classNames(
                        ...
                        'text-accent-500 hover:text-cude-textPrimary',
