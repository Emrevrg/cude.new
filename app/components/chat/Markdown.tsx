// Cude.new - Markdown.tsx (Cude product surface, 2026)
...
import {
  ...
  isArtifactMarker,
  isQuickActionMarker,
  isSelectedElementMarker,
  isThoughtMarker,
} from '~/lib/cude/pipeline/messageMarkers';
    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (isSelectedElementMarker(className)) {
            const messageId = node?.properties.dataMessageId as string;
            ...
            if (elementDataAttr) {
              try {
                ...
                cudeEventLog.error('pipeline', 'A selected element could not be read', e);
              }
            }
            ...
            return (
              <div className="bg-cude-background-depth-3 border border-cude-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-cude-background-depth-2 px-2 py-1 rounded text-cude-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-cude-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-cude-textSecondary !bg-cude-background-depth-2 border border-cude-borderColor p-2 rounded">
          ...
          if (isThoughtMarker(className)) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (isQuickActionMarker(className) || dataProps?.dataCudeQuickAction || dataProps?.datacudeQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            isQuickActionMarker(dataProps?.class?.toString()) ||
            dataProps?.dataCudeQuickAction === 'true' ||
            dataProps?.datacudeQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-cude-item-backgroundAccent text-cude-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
                ...
                onClick={() => {
                  if (type === 'file') {
                    ...
                    try {
                      ...
                      cudeEventLog.error('pipeline', 'A link in a message was not a usable URL', error);
                    }
                  }
                }}
              >
 ...
 * const input = "```xml\n<div class='__cudeArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__cudeArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  const marks = (line: string) => line.includes(ARTIFACT_MARKER);

  if (!content || !marks(content)) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex(marks);
