// Cude.new - UserMessage.tsx (Cude product surface, 2026)
/*
 * @ts-nocheck
          ...
          {profile?.avatar || profile?.username ? (
            <div className="flex items-end gap-2">
              <img
                ...
                decoding="sync"
              />
              <span className="text-cude-textPrimary text-sm">{profile?.username ? profile.username : ''}</span>
            </div>
          ) : (
  ...
  return (
    <div className="flex flex-col bg-accent-500/10 backdrop-blur-sm px-5 p-3.5 w-auto rounded-lg ml-auto">
      <div className="flex gap-3.5 mb-4">
        {images.map((item, index) => (
          <div className="relative flex rounded-lg border border-cude-borderColor overflow-hidden">
      ...
      {textContent.length > 1600 ? (
        <details className="min-w-0 max-w-full">
          <summary className="cursor-pointer break-words">{textContent.slice(0, 180)}… · Show full request</summary>
...
function stripMetadata(content: string) {
  const artifactRegex = /<cudeArtifact\s+[^>]*>[\s\S]*?<\/cudeArtifact>/gm;
