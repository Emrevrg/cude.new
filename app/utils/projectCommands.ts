// Cude.new - projectCommands.ts (Cude product surface, 2026)
...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<cudeAction type="shell">${commands.setupCommand}</cudeAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<cudeAction type="start">${commands.startCommand}</cudeAction>
`;
  }
...
<cudeArtifact id="project-setup" title="Project Setup">
${commandString}
</cudeArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeCudeArtifactTags(input: string) {
  const regex = /(<(?:cude|cude)Artifact[^>]*>)([\s\S]*?)(<\/(?:cude|cude)Artifact>)/g;
...
export function escapeCudeAActionTags(input: string) {
  const regex = /(<(?:cude|cude)Action[^>]*>)([\s\S]*?)(<\/(?:cude|cude)Action>)/g;
...
export function escapeCudeTags(input: string) {
  return escapeCudeArtifactTags(escapeCudeAActionTags(input));
}

// Legacy aliases — keep imports working
export const escapecudeArtifactTags = escapeCudeArtifactTags;
export const escapecudeAActionTags = escapeCudeAActionTags;
export const escapecudeTags = escapeCudeTags;
