// Cude.new - markdown.ts (Cude product surface, 2026)
...
import { ALL_MARKERS, THOUGHT_MARKER } from '~/lib/cude/pipeline/messageMarkers';
  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="${THOUGHT_MARKER}">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [...(defaultSchema.attributes?.div ?? []), 'data*', ['className', ...ALL_MARKERS]],
    button: [
