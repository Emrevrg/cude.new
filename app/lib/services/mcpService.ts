// Cude.new - mcpService.ts (Cude product surface, 2026)
import {
...
export class MCPService {
  ...
  async updateConfig(config: MCPConfig) {
    /*
     * The server names, not the configuration.
     *
     * A server entry carries an optional `headers` record, which is where the
     * `Authorization` for a remote MCP server goes — so serialising the whole
     * config wrote those tokens into the log. The same mistake the git proxy
     * made with its forwarded headers. Which servers were configured is the
     * diagnosable part.
     */
    logger.debug('Updating MCP servers', Object.keys(config.mcpServers ?? {}).join(', ') || '(none)');
    this._config = config;
