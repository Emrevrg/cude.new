// Cude.new - useGit.ts (Cude product surface, 2026)
...
const lookupSavedPassword = (url: string) => {
  ...
  try {
    ...
    logger.debug('Could not read the saved git credentials', error);
    return null;
  }
};
    ...
    async (url: string, retryCount = 0) => {
      ...
      try {
        ...
        if (retryCount > 0) {
          ...
          logger.debug(`Retrying the clone (attempt ${retryCount + 1})`);
        }
        ...
        await git.clone({
          ...
          headers,
          onProgress: (event) => {
            logger.debug('Clone progress', event);
          },
          onAuth: (baseUrl) => {
            let auth = lookupSavedPassword(baseUrl);

            if (auth) {
              logger.debug('Using saved credentials for', baseUrl);
              return auth;
            }

            logger.debug('Repository needs credentials', baseUrl);
          ...
          onAuthSuccess: (baseUrl, auth) => {
            logger.debug(`Credentials accepted by ${baseUrl}`);
