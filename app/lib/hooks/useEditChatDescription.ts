// Cude.new - useEditChatDescription.ts (Cude product surface, 2026)
import { useStore } from '@nanostores/react';
import { useCallback, useEffect, useState } from 'react';
import { toast } from 'react-toastify';
import {
  conversationId as chatIdStore,
  conversationDescription as descriptionStore,
...
export function useEditChatDescription({
  ...
  const fetchLatestDescription = useCallback(async () => {
    ...
    try {
      const conversation = await getConversationStore().get(chatId);
      return conversation?.description || initialDescription;
    } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        ...
        await getConversationStore().setDescription(chatId, currentDescription);
