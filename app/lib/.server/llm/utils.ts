// Cude.new - utils.ts (Cude product surface, 2026)
import { type Message } from 'ai';
import { DEFAULT_MODEL, DEFAULT_PROVIDER, MODEL_REGEX, PROVIDER_REGEX } from '~/utils/constants';
import { type FileMap } from './constants';
import { isSendableProjectPath } from '~/utils/fileUtils';
...
export function simplifyCudeActions(input: string): string {
  // Using regex to match cudeAction tags that have type="file"
  const regex = /(<cudeAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/cudeAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  const filePaths = Object.keys(files).filter(isSendableProjectPath);
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<cudeAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</cudeAction>`;
    });

  return `<cudeArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</cudeArtifact>`;
}
