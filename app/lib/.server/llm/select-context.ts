// Cude.new - select-context.ts (Cude product surface, 2026)
...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyCudeActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyCudeActions(content);

      content = content.replace(THOUGHT_BLOCK, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Cude failed to select files`);
  }
...
export function getFilePaths(files: FileMap) {
  return Object.keys(files).filter(isSendableProjectPath);
}
