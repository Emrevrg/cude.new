...
export function isReasoningModel(modelName: string): boolean {
  /*
   * A guess from the name, and only a fallback. What a model actually is comes
   * from the registry, on `ModelInfo.reasoning`; this is for the ones it has
   * never heard of — somebody's own gateway, a model released this morning.
   *
   * Guessing is genuinely unreliable: nothing in "moonshotai/kimi-k3" says it
   * reasons, and it does.
   */
